...
# Interactively add a task to Taskwarrior via read in a terminal + dmenu

if tty -s
then
    __menu_cmd() { menu.sh "slmenu" "PROJECT" ; }
elif [[ $DISPLAY ]]
then
    __menu_cmd() { menu.sh "dmenu2" "PROJECT" ; }
else
    exit 1
fi

echo "Choose a project, insert a new one or escape..."
project=$(task rc.list.all.projects=yes _projects | tee >(cut -d '.' -f1) | sort -u | __menu_cmd)
[[ ! $project ]] && { echo "No project has been chosen." 1>&2 ; exit 1 ; }
printf '%s\t%s\n' "Project:" "${project//./ > }"
